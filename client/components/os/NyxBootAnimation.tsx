...
interface NyxBootAnimationProps {
  onComplete: () => void
}

export const NyxBootAnimation: React.FC<NyxBootAnimationProps> = ({ onComplete }) => {
  const [phase, setPhase] = useState<'moon-drop' | 'logo-fade' | 'boot-sequence' | 'complete'>('moon-drop')
  ...
  useEffect(() => {
    const timeline = async () => {
      ...
      // Phase 2: Logo fades in with soft sound (2 seconds)
      await new Promise(resolve => setTimeout(resolve, 2000))
      setPhase('boot-sequence')
      
      // Phase 3: Boot sequence (1.5 seconds)
      ...
      <AnimatePresence>
        {phase === 'boot-sequence' && (
          ...
          >
            ...
            <div className="space-y-1 text-xs text-purple-300/60 font-mono">
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
