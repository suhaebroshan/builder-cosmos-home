      ...
      >
        <div className="text-center">
          ...
          <div className="space-y-3 text-white/80 text-sm leading-relaxed">
            <p>
              <strong>
                This is a demonstration, prototype, and proof of concept
              </strong>{" "}
              of what's possible with modern web technologies.
            </p>
            <p>
              Nyx OS showcases advanced UI/UX design, performance optimization,
              and cross-platform compatibility - all running entirely in your
              browser.
            </p>
            <p>
              While fully functional, this is not a production operating system.
              It's a glimpse into the future of web-based computing experiences.
            </p>
            <div className="mt-6 p-4 bg-blue-500/20 rounded-2xl border border-blue-400/30">
              <p className="text-blue-200 text-xs">
                💡 <strong>Pro tip:</strong> Try switching between device modes,
                test the apps, and enjoy the adaptive performance system that
                scales with your device capabilities!
              </p>
            </div>
          </div>
          <button
...
export const NyxOS: React.FC = () => {
  const { currentEmotion, emotionIntensity, addMessage, setEmotion } =
    useSamStore();
  ...
  const selectedDeviceType: DeviceType = isPhone
    ? "phone"
    : isTablet
      ? "tablet"
      : "desktop";
  ...
  const forcePhoneView =
    window.innerWidth < 768 ||
    (window.innerWidth < 900 && "ontouchstart" in window);
  const forceMobileView =
    forcePhoneView ||
    (window.innerWidth < 1200 && navigator.maxTouchPoints > 0);
  const actualDeviceType = forcePhoneView
    ? "phone"
    : forceMobileView && selectedDeviceType === "desktop"
      ? "tablet"
      : selectedDeviceType;
  ...
  const handleBootComplete = (user: User) => {
    ...
    setTimeout(() => {
      ...
      switch (actualDeviceType) {
        case "phone":
          welcomeMessage +=
            "Your Samsung One UI 7 style mobile experience is ready with smooth gestures and dynamic wallpapers!";
          break;
        case "tablet":
          welcomeMessage +=
            "Your hybrid tablet experience combines mobile smoothness with desktop functionality!";
          break;
        case "desktop":
          welcomeMessage +=
            "Your full desktop experience is ready with advanced multitasking and performance monitoring!";
          break;
      }
  ...
  useEffect(() => {
    ...
    const handleChangeWallpaper = () => {
      ...
      addMessage(
        "Dynamic wallpaper system activated! Experience different themes in dark and light modes.",
        "sam",
        "excited",
      );
    };
  ...
  return (
    <OptimizedShutdownSystem>
      ...
      >
        ...
        {actualDeviceType === "desktop" && <PerformanceMonitor />}
        ...
        {actualDeviceType === "desktop" && (
          <motion.div
            ...
            transition={{
              delay: profile.useReducedMotion ? 0.2 : 0.4,
