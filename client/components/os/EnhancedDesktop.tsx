...
export const EnhancedDesktop: React.FC = () => {
  const { openWindow, windows, minimizeWindow, focusWindow, getWindowsByApp } =
    useWindowStore();
  const {
    icons,
    folders,
  ...
  const { deviceInfo, uiConfig, isPhone, isTablet, isDesktop } =
    useDeviceDetection();
  ...
  const [editModeTimeout, setEditModeTimeout] = useState<NodeJS.Timeout | null>(
    null,
  );
  ...
  useEffect(() => {
    if (icons.length === 0) {
      ...
      const iconsPerRow = Math.floor(
        (window.innerWidth - padding * 2) / gridSize,
      );
      ...
      const defaultApps = [
        ...
        },
        {
          ...
          defaultPosition: { x: 200, y: 100 },
          description:
            "Full-featured document editor with formatting and export",
        ...
        },
        {
          ...
          description:
            "Create stunning presentations with slides and animations",
  ...
  const handleDesktopLongPress = (e: React.MouseEvent) => {
    if (e.target === e.currentTarget) {
      ...
      addMessage(
        "Edit mode activated! You can now move, resize, and organize your apps, bruv.",
        "sam",
        "focused",
      );
    }
  ...
  const openApp = (icon: any) => {
    ...
    if (isPhone && appWindows.length >= uiConfig.maxWindows) {
      addMessage(
        "Window limit reached for mobile mode. Close some apps first!",
        "sam",
        "annoyed",
      );
      return;
    ...
    if (isPhone) {
      ...
      windowPosition = {
        x: uiConfig.windowPadding || 8,
        y: uiConfig.statusBarHeight || 32,
      };
    }
    ...
    if (icon.appId === "sam-chat") {
      setTimeout(() => {
        ...
        addMessage(
          "Hey there! Welcome to Nyx OS, bruv. What's good?",
          "sam",
          "happy",
        );
      }, 500);
    } else if (
      icon.appId === "runner-game" ||
      icon.appId === "flappy-game" ||
      icon.appId === "chess-game"
    ) {
      setTimeout(() => {
        setEmotion("excited", 0.7);
        addMessage(
          `Time to game! ${icon.name} is fucking sick, bro. Let's see what you got.`,
          "sam",
          "excited",
        );
      }, 500);
      ...
      setTimeout(() => {
        setEmotion("focused", 0.8);
        addMessage(
          "Memory time! I'll help you remember everything and keep you on schedule, my dude.",
          "sam",
          "focused",
        );
      }, 500);
      ...
      setTimeout(() => {
        ...
        addMessage(
          "New browser loaded! This one's got all the goods - tabs, bookmarks, history, the works!",
          "sam",
          "excited",
        );
      }, 500);
      ...
      setTimeout(() => {
        ...
        addMessage(
          "Time to write! This notepad has everything - formatting, search, export. Write your masterpiece!",
          "sam",
          "focused",
        );
      }, 500);
      ...
      setTimeout(() => {
        ...
        addMessage(
          "Calculator ready! Basic or scientific - I got you covered for all the math.",
          "sam",
          "focused",
        );
      }, 500);
      ...
      setTimeout(() => {
        setEmotion("excited", 0.8);
        addMessage(
          "2048 time! This puzzle is addictive as hell, bro. Can you reach the magical 2048 tile?",
          "sam",
          "excited",
        );
      }, 500);
  ...
  const snapToGrid = (
    x: number,
    y: number,
    _iconSize: { width: number; height: number } = { width: 64, height: 64 },
  ) => {
  ...
  const handleIconDragEnd = (iconId: string, info: PanInfo) => {
    ...
    const statusBar =
      uiConfig.statusBarHeight || (isPhone ? 32 : isTablet ? 28 : 0);
  ...
  const handleCreateFolder = () => {
    if (selectedIcons.length >= 2) {
      ...
      if (firstIcon) {
        ...
        setEmotion("excited", 0.7);
        addMessage(
          "Folder created! Nice organization skills, my man.",
          "sam",
          "excited",
        );
      }
