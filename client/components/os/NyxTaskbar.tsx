...
export const NyxTaskbar: React.FC = () => {
  const { windows, focusedWindowId, minimizeWindow, focusWindow, openWindow } =
    useWindowStore();
  ...
  const getBatteryColor = () => {
    ...
    if (systemStats.batteryLevel <= 0.3) return "text-yellow-400";
    return "text-white";
  };
  ...
  const quickActions = [
    {
      ...
      label: "Settings",
      action: async () => {
        const { Settings: SettingsApp } = await import(
    ...
    {
      icon: systemStats.wifi ? Wifi : WifiOff,
      label: systemStats.wifi ? "Connected" : "Offline",
      action: () => {
        // Toggle network status simulation
        setSystemStats((prev) => ({ ...prev, wifi: !prev.wifi }));
        addMessage(
          systemStats.wifi ? "Network disconnected" : "Network connected",
          "sam",
          "neutral",
        );
      },
    ...
    {
      icon: systemStats.bluetooth ? Bluetooth : BluetoothOff,
      label: systemStats.bluetooth ? "Bluetooth On" : "Bluetooth Off",
      action: () => {
        // Toggle bluetooth status simulation
        setSystemStats((prev) => ({ ...prev, bluetooth: !prev.bluetooth }));
        addMessage(
          systemStats.bluetooth ? "Bluetooth disabled" : "Bluetooth enabled",
          "sam",
          "neutral",
        );
      },
  ...
  const frequentApps = [
    ...
    },
    {
      ...
      label: "Media Player",
      action: async () => {
        const { WindowsMediaPlayer } = await import(
          "@/components/apps/WindowsMediaPlayer"
        );
        openWindow({
          appId: "media-player",
  ...
  const notifications = [
    {
      id: 1,
      title: "System Update",
      message: "Nyx OS 1.1 is available",
      time: "2m ago",
      type: "system",
    },
    {
      id: 2,
      title: "AI Assistant",
      message: "Voice recognition optimized",
      time: "5m ago",
      type: "ai",
    },
    {
            ...
            <div className="space-y-3">
              ...
              {systemStats.memoryUsage && (
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-white text-sm">
                    {formatBytes(systemStats.memoryUsage.usedJSHeapSize)} /{" "}
                    {formatBytes(systemStats.memoryUsage.totalJSHeapSize)}
                  </div>
                </div>
              )}
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-white/5 rounded-lg border border-white/10">
            ...
            {React.createElement(getBatteryIcon(), {
              className: cn("w-3.5 h-3.5", getBatteryColor()),
            })}
          </div>
      ...
      <AnimatePresence>
        {isMoonMenuOpen && (
          ...
          >
              ...
              <motion.button
                ...
                onClick={() => {
                  window.dispatchEvent(
                    new CustomEvent("nyx:toggle-command-palette"),
                  );
            ...
            {/* Quick Actions */}
            <h3 className="text-white/80 font-semibold text-xs mb-2 uppercase tracking-wider">
              System
            </h3>
            ...
            <div className="border-t border-white/10 pt-3 mt-3">
              <motion.button
                ...
                onClick={() => {
                  const newMode =
                    themeSettings.mode === "dark" ? "light" : "dark";
                  setThemeMode(newMode);
                  addMessage(
                    `${newMode === "dark" ? "🌙" : "☀️"} Switched to ${newMode} mode`,
                    "sam",
                    "happy",
                  );
                  setIsMoonMenuOpen(false);
            ...
            <div className="mt-3 pt-3 border-t border-white/10">
              <motion.button
                ...
                onClick={() => {
                  setEditMode(true);
                  addMessage(
                    "✏️ Desktop customization enabled",
                    "sam",
                    "focused",
                  );
                  setIsMoonMenuOpen(false);
